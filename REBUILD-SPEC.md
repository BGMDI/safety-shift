...
### 5.4 ⚠️ الحارس `RolesGuard` يسمح افتراضياً

**`RolesGuard` يُرجع `true` إذا لم يكن على المسار `@Roles()`.** أي `@Get` بلا `@Roles` **مقروء لأي مستخدم مسجّل دخول**.

> هذا تسبّب فعلياً في تسريب: أي موظف كان يقرأ كل الرواتب وأرقام الهويات حتى أُغلق.

**القاعدة الإلزامية:** كل مسار قراءة حساس **يجب** أن يحمل `@Roles(...)` صريحاً أو فحص `assertSelfOr*`. لا تعتمد على الحارس ليمنع افتراضياً.
...
1. **`RolesGuard` يسمح افتراضياً** — كل مسار حساس يحتاج `@Roles()` صريحاً أو `assertSelfOr*`.
...
| تسريب الرواتب والهويات | `@Get` بلا `@Roles` + الحارس يسمح افتراضياً | `@Roles` صريح على كل مسار حساس |
...
**الدليل:** يُرجع `true` عند غياب `@Roles()` — أي **السماح افتراضي**.

**الأثر:** نسيان `@Roles` على مسار جديد = تسريب صامت. **حدث فعلاً** — كل الرواتب وأرقام الهويات كانت مقروءة لأي موظف.

**التوصية:** اقلب المنطق — **المنع افتراضي**، والاستثناء صريح:
```ts
@Public()                    // ديكوريتر صريح للمسارات العامة
@SelfOrRoles('hr_manager')   // أو: سجله الشخصي أو دور محدد
// أي مسار بلا ديكوريتر ⇒ مرفوض
```
الخطأ حينها يكون **إغلاقاً زائداً** (يُكتشف فوراً في الاختبار) لا **تسريباً صامتاً**.
...
### 11.5 `AuditLog` شبه معطّل 🟠

**الدليل الرقمي:** `prisma.auditLog.create` مستدعاة من **ملف واحد فقط** (`attendance.service.ts`، موضعان) من أصل **21 وحدة**.

**الأثر:** قسم "سجل التدقيق" في الواجهة يقرأ جدولاً لا تكتب فيه 20 وحدة من 21. إنشاء موظف، تعديل راتب، حذف شفت، اعتماد إجازة — **لا شيء منها مسجَّل**. السجل يوحي بتغطية شاملة وهو لا يغطي إلا الحضور.

**التوصية:** `AuditInterceptor` عام يلتقط كل `POST/PUT/DELETE` تلقائياً، بدل استدعاء يدوي في كل خدمة:
```ts
@Injectable()
export class AuditInterceptor implements NestInterceptor {
  // يسجّل: من · ماذا · أي وحدة · القيمة قبل/بعد · IP — تلقائياً
}
```
